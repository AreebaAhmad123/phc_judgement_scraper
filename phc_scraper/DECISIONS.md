...
- **Correction:** the section above assumed Groq (matching the code's
  default `METADATA_LLM_MODEL`); this project's actual `.env` has
  `LLM_PROVIDER=gemini` / `METADATA_LLM_MODEL=gemini-2.5-flash`. That
  matters because Gemini calls go through a completely different code
  path (`llm_client._gemini_native_call`, raw REST via `?key=`, see that
  module's docstring) than Groq/OpenAI (`_openai_compat_call`, the
  `openai` SDK) - so `openai.RateLimitError` never fires for Gemini
  calls, and `LLMQuotaExhausted` above would have stayed dead code for
  this project's actual configuration. Fixed: `llm_client.py` now raises
  `LLMRateLimitError` on a 429 from the Gemini REST path, and
  `llm_metadata_extractor.py` catches `(RateLimitError, LLMRateLimitError)`
  together, so both provider paths reach the same
  retry-then-`LLMQuotaExhausted` behavior. `scripts/submit_metadata_batch.py`
  / `collect_metadata_batch.py` are still Groq-specific (Groq's Batch API
  endpoints/format) and do NOT work against Gemini - not used for now,
  since the immediate need is a ~100-judgment demo run, comfortably
  within Gemini 2.5 Flash's free tier (~10 RPM, ~250-1,500 RPD depending
  on when Google's docs are checked - varies by source/date). A Gemini
  equivalent of the batch scripts would need to be written separately if
  the full ~5,700-judgment corpus needs backfilling later.
...
- `judge` filtering/enrichment coverage depends entirely on how much of
  ...
  --- /home/claude/orig/DECISIONS.md	2026-07-16 18:16:22.000000000 +0000
 /home/claude/project/DECISIONS.md	2026-07-17 19:21:15.903485333 +0000
@@ -100,6 +100,44 @@
    cases, the very next real scrape run will discover and add it, because
    of point (4) above.
 
6. **Bug found in point (3)'s implementation, and why the fix stays
   local-only.** `download_pdf()`'s SC branch computed
   `dest_dir = config.SC_PDF_DIR` and then never used it — both branches
   called the same `_brief_dest_path()`, which only ever resolves through
 `naming.local_pdf_path()` → `config.PDF_DIR`. So SC judgments had
   actually been landing in the flat `pdfs/` folder all along, silently
   contradicting point (3) above. Fixed by giving `_brief_dest_path()` a
   real `dest_dir` parameter and passing `config.SC_PDF_DIR` for the SC
   branch (`pdf_downloader.py`); `migrate_existing_pdfs.py` had the same
   bug (`target_path = local_pdf_path(url)` ignored `is_sc`) and got the
   equivalent fix. A one-off `scripts/fix_sc_pdf_locations.py` moves
   already-misplaced files into `pdfs/sc_judgments/` and updates
   `sc_judgment_local_pdf_path` to match — no re-download needed, since
   the PDF content itself was always correct, only its folder was wrong.

   This split is **local-disk-only and does not reach S3 or Drive**.
   `naming.s3_key()` builds every S3 key from `court.s3_subfolder` + the
   filename stem alone (`{artifact}/{court.s3_subfolder}/{encoded}`) —
   it never references `PDF_DIR`/`SC_PDF_DIR`, and `s3_uploader.upload_file()`
   takes the key as an argument rather than deriving it from the local
   path. `gdrive_upload.py` is the same: it takes an absolute path + a
   sha256 and uploads into one flat Drive folder, with no branching on
   local directory or `sc_judgment` status. So this fix does not add a
   parallel `sc_judgments/` prefix on S3 — it can't, since S3 key
   construction doesn't consult the local path at all. That's also brief-
   consistent: Section 9.3's S3 key hierarchy is fixed at four top-level
   prefixes with one court subfolder each, and doesn't define a fifth
   prefix for SC judgments (the whole SC-judgment concept is outside
   Task 3's brief scope — see point 1 above — so Section 9.3 was never
   written with it in mind; keeping S3 flat is the reading that stays
   inside the brief rather than extending it).

   Rollback, if ever needed: because the split is local-only, undoing it
   is just moving the files in `pdfs/sc_judgments/` back to `pdfs/` and
   restoring `sc_judgment_local_pdf_path` to the old flat relative path
   — no S3 keys, no Drive files, and no MongoDB records are touched
   either way.
