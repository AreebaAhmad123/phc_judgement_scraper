"""Pre-seed pdfs/ from PDFs you already have in another folder, so a
...
from phc_scraper.naming import local_pdf_path, pdf_leaf
...
def _normalize(name):
    """Lowercase, strip extension, drop every non-alphanumeric character.
    Used to compare a URL leaf ('2026PHC153') against a source filename
    that might be '2026 PHC 153 (final).pdf', '2026_PHC_153.pdf', etc."""
...
def migrate(source_dir, years, dry_run, min_key_len, allow_ambiguous_guess):
    ...
    stats = {"already_present": 0, "matched_copied": 0, "matched_rejected_not_pdf": 0,
              "skipped_too_generic": 0, "skipped_ambiguous": 0, "matched_ambiguous_forced": 0,
              "unmatched": 0}
    unmatched_urls = []
    seen = set()
    ...
    for field, url in _iter_pdf_urls(years):
        ...
        target_path = local_pdf_path(url)
        ...
        pool = sc_index if field == "sc_judgment_pdf_url" else main_index
        ...
        if dry_run:
            logger.info("[dry-run] Would copy %s -> pdfs/%s", chosen, target_name)
        else:
            os.makedirs(os.path.dirname(target_path), exist_ok=True)
            shutil.copy2(chosen, target_path)
            logger.info("Seeded %s <- %s", target_name, chosen)
        stats["matched_copied"] += 1

    logger.info(
        "Migration %s: already_present=%d matched_copied=%d matched_ambiguous_forced=%d "
        "skipped_too_generic=%d skipped_ambiguous=%d rejected_not_pdf=%d unmatched=%d",
        "(dry-run)" if dry_run else "complete",
        stats["already_present"], stats["matched_copied"], stats["matched_ambiguous_forced"],
